...
import java.text.SimpleDateFormat;
 ...
 class Day10Bank
{
	static HashMap<Integer,Day10Account> Account = new HashMap<>();       //Accno, Account
	...
	public void credit(int accountNo)
	{
		...
		try 
		{
			...
			Day10Account acc = Account.get(accountNo);
			...
			if(camount>0)
			{
				...
				Day10Account.miniState.add("Account "+accountNo+" is credited with Rs"+camount+" on date "+date+" at "+time);
			}
			else
			...
			try 
			{
				...
				if(damount>0)
				{
					...
					else 
					{
						...
						Day10Account.miniState.add("Account "+accountNo+" is debited with Rs"+damount+" on date "+date+" at "+time);
...
public class Day10Account extends Day10Bank
{
	...
	Day10Account(String name,int no,int bal)
	{
		accHolderName = name;
		accountNo = no;
		accountBal = bal;
		...
		{							//again and again
			...
			{	
				...
				switch(ch)
				{
					...
					case 3: for(int i=0;i<Day10Account.miniState.size();i++)
								System.out.println(Day10Account.miniState.get(i));
		...
		{								//can again perform the input (the program doesn't terminate)
			try 
			{
				...
				else
				{
					...
					Day10Account accob = new Day10Account(name,num,bal);
					accob.call();
				}
			} 
			catch (NumberFormatException | IOException e) 
